...
{
    /*Creating temproary buffer temp for device */
...
/******************************
 * @breief Changes the device access  parameters
 *
 *****************************/
...
{
    /* Allocate the  region for the device */
    alloc_chrdev_region(&dev_num, 0, 1, DEVICE_NAME);
    major = MAJOR(dev_num);
    cdev_init(&my_cdev, &fops);
    cdev_add(&my_cdev, dev_num, 1);

    /* Create the char device with the specified name */
...
{
    /* Removing device with /dev */
    device_destroy(pClass, dev_num);
    class_destroy(pClass);

    /* Free the device region */
    cdev_del(&my_cdev);
